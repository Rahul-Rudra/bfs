...
class StoreSearchComponent extends Component {
    ...
    oms.addListener('format', function (marker, status) {
      var iconURL =
        status === OverlappingMarkerSpiderfier.markerStatus.SPIDERFIED
          ? require('./img/icons8-google-maps.png').default
          : status === OverlappingMarkerSpiderfier.markerStatus.SPIDERFIABLE
            ? require('google-maps-Plus.png').default
    ...
    return (
      ...
      <div className="col-12 bg-gray px-0 store_locator_mein midcontent">
        ...
        <div className="store_locator py-sm-4  py-md-5">
          <div className="px-0 pt-5 pt-md-0 container">
            ...
            <div className="search_frm  pb-4 d-md-flex align-items-center">
              <div className="input-group mb-3 mb-md-0">
                <Autocomplete
                  ...
                  onChange={(e) => this.searchChangeHandler(e)} style={{width: '100%'}}
                  onPlaceSelected={(place) => this.getLatLon(place)}
                  id="searchfield"
                />
              </div>
              <div className="ml-auto">
                <div className="my-lyc">
                  <button
                    onClick={() => this.getCurrentLocationOnClick()}
                    className="btn text-uppercase bg-info px-4 py-3 d-inline-block d-none-mob  text-white sp-shadow"
                  >
                    Use My location
                  </button>
                </div>
            ...
            <div className="dropdown_adv position-relative clearfix text-left">
              ...
              }} className="dropdown-toggle-btn" style={{cursor: 'pointer'}} type="button">
                Advanced Search <img className="logo_size" src={down_arrow} alt="arrow here"/>
              </button>
              <div
                className={this.state.toggleDropdown ? 'show custom_drop_field view_productList' : 'show custom_drop_field'}
                aria-labelledby="about-us" id="advanceSearch">
                <div className="d-flex  inline_add">
                  ...
                  <span onClick={() => {
                    this.addToggleDropdown()
                  }} className="close_p_menu ml-auto">
                  ...
                  <div className="col-md-6 pr-lg-0">
                    ...
                    <label className="check ">Engineered Wood Products
                      <input type="checkbox" name="is_name"
                             onChange={() => this.handleChange('engineeredWoodProducts')}/>
                      <span className="checkmark"></span>
                    </label>
                    ...
                    <label className="check ">Interior Trim & Moduling
                      <input type="checkbox" name="is_name"
                             onChange={() => this.handleChange('interiorTrimMouldings')}/>
                      <span className="checkmark"></span>
                    </label>
          ...
          <button id="allowLocationClick" onClick={() => this.allowLocationClicked()}
          ...
          <div className="row m-0 mt-md-5 mt-lg-5 mt-sm-0 map-section">
            <div className="col-sm-12 col-md-12 col-lg-5 contpost px-0 order-lg-1 order-2">
              {!this.state.showBlur && this.state.storeData.length === 0 ? (
                <h5 className="w-100 text-center p-5 mb-0 no-result ">No stores found matching your location or
                  criteria</h5>) : null}
              <div className="scrollbar_store" id="storelocatorlist">
                  ...
                  this.state.paginateData.length > 0 ?
                    this.state.paginateData.map((store, index) => (
                      <div id={'store' + store.$id}
                           className={this.state.showBlur === true ? 'col-12 py-3 hideele' : this.state.selectedPlace.markerId === store.$id ? 'col-12 py-3 selected-store-class' : 'col-12 py-3'}
                           key={index}>
                        <div className={this.state.selectedPlace.markerId === store.$id ? 'row ' : 'row'}>
                          <div className="col-xl-5 mb-3 mb-md-0">
                            <Link onClick={() => {
                              this.selectThisLocation();
                            }} className="thumbnail" to={'/location_' + store.Alias}>
                              {store.LargeImage ? (<img onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = defaultimage
                              }} className="w-100"
                                                        src={globalVar.base_url + store.LargeImage + '?width=195&mode=max&animationprocessmode=first'}
                                                        alt="store here"/>) : (
                          ...
                          <div className="col-xl-7">
                            <h5 className="font-weight-bold mb-4 mt-2 position-relative">
                              <span className="d-inline-block">{store.$id}.  </span> <Link onClick={() => {
                              this.selectThisLocation();
                            }} className="text-dark store-name" to={'/location_' + store.Alias}>{store.Name}</Link>
                            </h5>
                            ...
                            <Link onClick={() => {
                              this.selectThisLocation();
                              this.selectedLocation(store.Alias, store.Id, store.Name)
                            }}
                                  className="btn theme-btn text-uppercase wid-48 px-4 py-3 mb-3 d-inline-block login-transparent store-location__select-btn"
                                  to={'/location_' + store.Alias}>Select</Link>
                            <a
                              className="btn theme-btn text-uppercase wid-48 btn-outline-dark px-4 py-3 mb-3 d-inline-block login-red text-white store-location__quote-btn"
                              onClick={() => this.quoteOpenUrl(store.Id)}>
                              Quote</a>
                          </div>
                        </div>
                      </div>
                ...
                {!this.state.showBlur && this.state.storeData.length > 3 && this.state.viewMoreStores ? (
                  <div className="moreBtn text-center py-4">
                    <a
                      className="btn theme-btn text-uppercase wid-100 btn-outline-dark px-4 d-inline-block login-red text-white store-location__quote-btn"
                      onClick={() => {
                        count = count + 1;
                        this.viewMore(this.state.storeData)
                      }}>
                      View More Stores
                    </a>
                  </div>
            ...
            <div
              className={this.state.showBlur === true ? ' col-sm-12 col-md-12 col-lg-7 map_locate   contpost hideele over-flow' : ' col-sm-12 bg-gray col-md-12 col-lg-7 map_locate contpost over-flow pull-right order-lg-2 order-1'}>
        ...
        <Modal isOpen={this.state.modal} toggle={this.toggle} className="request-form modal-dialog-centered">
          ...
          <ModalBody className="px-4 pt-4 px-md-5 pt-md-5 pb-0">
            <div className="row  m-0">
              ...
              <div className="form-group col-md-6  mb-5">
                <div className="position-relative">
                  <input id="zipcode" type="text" maxLength="5" onChange={() => this.handleZipcodeChange()}
                         className="form-control border-top-0  border-left-0 border-right-0 effect-2 mb-0 bg-transparent rounded-0 border-top-0  border-left-0 border-right-0 px-0"
                         autoComplete="off"/>
              ...
              <div className="form-group col-md-6  mb-5">
                <div className="position-relative">
                  <input id="phone" type="text" maxLength="12" onChange={() => this.handlePhoneChange()}
                         className="form-control effect-2 mb-0 bg-transparent rounded-0 border-top-0  border-left-0 border-right-0 px-0"
                         autoComplete="off"/>
        ...
        <Modal isOpen={this.state.allowgps} toggle={this.togglelocation} className="modal-dialog-centered location">
          <ModalHeader toggle={this.togglelocation} className="pb-0"><span
            className="allow-loc-head h1 m-auto pb-0 d-block font-weight-light ">Allow Location Access</span></ModalHeader>
          <ModalBody className="">
            <p className="h5 font-weight-light line-height">If you would like to be able to find stores close to you,
              please enable GPS or allow location sharing. </p>
          ...
          <ModalFooter className="allow-loc-foot">
            ...
            <Button className="bg-info btn text-uppercase theme-btn  px-4 py-3 btn btn-secondary" color="primary"
                    onClick={this.allowCurrentLocation}>Allow</Button>{' '}
            <Button className="btn btn-danger text-uppercase theme-btn  px-4 py-3 btn btn-secondary" color="secondary"
                    onClick={this.togglelocation}>Cancel</Button>
          </ModalFooter>
        </Modal>
 ...
 * Define the protype
 */
StoreSearchComponent.propTypes = {
